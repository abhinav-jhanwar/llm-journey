...
Backpropagation in neural net is propagating backwards across all the nodes starting the output node to set the gradient for them. The gradient decides how much it effects the output which is why the gradients are derivates of the output in relation to them. This is exactly the same concept as chain rule in derivates. This helps in training a neural network to change it's weights and biases to get the desired output.
...
- During backprop, we need to add the gradients to itself so that when we work multiple times with the same variable, the gradient is adjust accordingly.
...
- It is just the chain rule for derivates, so it's important to know this.
